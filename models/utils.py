...
def conectar():
    ...
    """
    conn = sqlite3.connect(path.join(path.expanduser('~'), 'cartoespreaprovados.db'), detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
...
def listar_clientes_telefone():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute('SELECT cli.nome, cli.cpf, telefones.numero, envio.id_cliente FROM telefones, clientes as cli, envio WHERE envio.id_cliente = cli.id AND telefones.id_cliente = cli.id and telefones.whatsapp = 1 GROUP BY telefones.numero')
    clientes = cursor.fetchall()
    lista_clientes = []
    if len(clientes) > 0:
        for cliente in clientes:
